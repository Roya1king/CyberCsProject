...
const styledNodes = useMemo(() => {
    return nodes.map((node) => {
        ...
        const baseStyle = {
            ...
            borderStyle: 'solid',
            borderWidth: '2px',
            color: '#fff', // Make label text white for readability on dark background
            padding: '10px 20px',
            borderRadius: '8px',
        };

        if (!selectedNodeId) {
            // Default view: no node is selected
            return {
                ...node,
                style: {
                    ...baseStyle,
        ...
        // Highlight view: a node is selected
        ...
        return {
            ...
            style: {
                ...baseStyle,
                // Use highlight color for the border if the node is highlighted
                borderColor: isHighlighted ? HIGHLIGHT_COLOR : BASE_COLOR,
                // Dim the node if it's not part of the selection
                opacity: isHighlighted ? BASE_OPACITY : DIMMED_OPACITY,
            },
        };
    });
